...
camera_frame_t* CAM_RecordCurrentFrame(void)
{
    ...
    newFrame->next = 0;
    newFrame->time = (uint)([EngineContext instance].timer.simulationTime);
...
void CAM_UpdateOrientationMatrix(void)
{
    ...
    if (camera.recording)
    {
        cameraAccTime += [EngineContext instance].timer.timediff;
...
void CAM_StartPlaying(NSString *filename) {
	...
	[EngineContext instance].timer.simulationTime = camera.path.frames[0].time;
